(function() {
	...
	function ScheduleTemplate( element ) {
		...
		this.timelineStart = getScheduleTimestamp(this.timelineItems[1].textContent);
		this.timelineUnitDuration = getScheduleTimestamp(this.timelineItems[2].textContent) - getScheduleTimestamp(this.timelineItems[1].textContent);
	...
	ScheduleTemplate.prototype.placeEvents = function() {
		...
		for(var i = 0; i < this.singleEvents.length; i++) {
			...
			if (anchor.getAttribute('data-start') != null && anchor.getAttribute('data-end') != null ){
			...
			var start = parseInt(anchor.getAttribute('class-start')),
				duration = parseInt(anchor.getAttribute('class-end')) - start;
				end = parseInt(anchor.getAttribute('class-end'));
				if (start > 5 ){
					var eventTop = (start - 1) * 50 + 50,
					eventHeight = (duration * 100 + 100)/2;
				}
				else {
					var eventTop = (start - 1) * 50,
					eventHeight = (duration * 100  + 100)/2;
				}
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		if(target.getAttribute('data-start') != null || target.getAttribute('data-end') != null)
		{
			this.modalDate.textContent = target.getAttribute('data-start')+' - '+target.getAttribute('data-end');
		}
		else {
			this.modalDate.textContent = ("Tiết " + target.getAttribute('class-start')+' - Tiết '+target.getAttribute('class-end'));
		}

		this.modal.setAttribute('data-event', target.getAttribute('data-event'));

		//update event content
		this.loadEventContent("../tracuulich/infodongtkb?id=" + target.getAttribute('data-content'));
