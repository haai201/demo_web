...
; (function ($, window, document, undefined) {
    ...
    function branchNoLine(list) {
        var html = [];
        html.push('<div class="hortree-branch-no-Line">');
        $(list).each(function () {
    ...
    function branchList(elem) {
        ...
        if (typeof elem !== 'undefined') {
            html.push('<div class="hortree-entry" data-entry-id="' + entryIdIndex + '" style="--data-y:'+elem.HocKy+'">');
        }
        else {
        ...
        if (typeof elem !== 'undefined') {
            if (elem.tooltip && elem.tooltip.toString().trim() !== '') {
                ...
                html.push(elem.TenHocPhan);
                html.push('</div>');
                
            } else {

                html.push('<div class="hortree-label status-' + elem.Status +'">' + elem.TenHocPhan + '</div>');

            }
        }
        if (typeof elem !== 'undefined' && typeof elem.children !== 'undefined' ) {
            if (elem.children.length) {
                html.push(branch(elem.children));
            }
        }
        if (typeof elem !== 'undefined' && typeof elem.fakechild !== 'undefined') {
            if (elem.fakechild.length) {
                html.push(branchNoLine(elem.fakechild));
            }
        }
    ...
    function assignEntryHeight() {
        $('.hortree-wrapper>.hortree-branch>.hortree-entry').each(function () {
            var max = 0;
            $(this).find('.hortree-label').each(function () {
                var h = $(this).height();
                if (h > max) {
                    max = h;
                }
            ...
            $(this).find('.hortree-label').each(function () {
                $(this).parent('.hortree-entry').height(max);
            });
        });
    }
